...
public class OptimizerServlet extends HttpServlet
{
	...
	protected void performNonOptimizationRelatedAction(HttpServletRequest request, HttpServletResponse response, String action) throws IOException {
		if (action.equalsIgnoreCase("saveInputXml")) {
			...
			loge("FIXME: unknow action=" + action);
		}
	}

	protected void performOptimization(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try{
			...
			final boolean clientOld = compareVersion(currVersion,clientVersion) != 0;
			if(clientOld){
			...
			if (solved) {
				if (esrGeneralProperties.esr_enabled)
					logw("CHECMKE: esrGeneralProperties.esr_enabled=true");
				if (pumpGeneralProperties.pump_enabled)
					logw("CHECMKE: pumpGeneralProperties.pump_enabled=true");
		...
		{
			loge("Exception e = " + e.getMessage());
			loge("Exception e = " + e.toString());
			e.printStackTrace();
			...
			if (e.getMessage() != null)
				...
				loge("FIXME: Critical error in the server");
			out.print(error);
		}
	}
